...
public class Main {
	...
	public static void main(String[] args) {
		...
		//GeradorIntenso g1 = new GeradorIntenso(30, 10, 7);
		GeradorLeve g1 = new GeradorLeve (30, 10);
		...
		while ((g1.listTarefasEspera.size() > 0 || numeroTarefasEmExecucao > 0) && count < 1000) {
			...
			for (int cpu = 0; cpu < listCPU.size(); cpu++) {
				...
				//		System.out.println("Tarefa adicionada na hora "+tarefa.getHora()+" com tempo de duracao "+tarefa.getTempodeCPU() +" e terminada no instante " + HoraAtual );
						...
						int cpuatual = cpu;
						for(int cpu1 = 0; cpu1 < listCPU.size(); cpu1++){
							transmitida++;
							if(cpu1!=cpuatual)
							{	
								if(listCPU.get(cpu1).TarefasDaCPU.size()!=0){
					//				System.out.println("---Roubei uma tarefa---"+" disse a cpu "+cpuatual);
									listCPU.get(cpuatual).TarefasDaCPU.add(listCPU.get(cpu1).TarefasDaCPU.get(0));
									listCPU.get(cpu1).TarefasDaCPU.remove(0);
									pegoutarefa = 1;
								}
							}
							if(pegoutarefa == 1){
